...
class MainActivityViewModel(petApi: PetApi.PetApiClient? = null) : ViewModel() {
    ...
    fun searchPets(): Observable<List<Pet>> {
    ...
    private fun loadPets(): Observable<List<Pet>> {
        return petApiClient.searchPet().subscribeOn(Schedulers.io()).map { t: RootResponse -> t.petfinder.pets.pet }
    }
}
